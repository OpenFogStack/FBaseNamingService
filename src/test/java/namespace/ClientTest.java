...
public class ClientTest {
	...
	private void createClient(ClientConfig c)
			throws IllegalArgumentException, InterruptedException {
		assertFalse("Client not active at start", controller.exists(activePath + c.getClientID()));
		assertFalse("Client not in tombstoned at start",
				controller.exists(tombstonedPath + c.getClientID()));

		@SuppressWarnings("unchecked")
		Response<Boolean> response = (Response<Boolean>) TestUtil.run(Command.CLIENT_CONFIG_CREATE,
				c, sender, controller);
		...
		assertFalse("Client not in tombstoned",
				controller.exists(tombstonedPath + c.getClientID()));
	}

	private void readClient(ClientID id, ClientConfig expected)
			throws IllegalArgumentException, InterruptedException {
		assertTrue("Client started in active", controller.exists(activePath + id));

		@SuppressWarnings("unchecked")
		Response<String> response =
				(Response<String>) TestUtil.run(Command.CLIENT_CONFIG_READ, id, sender, controller);
	...
	private void updateClient(ClientConfig original, ClientConfig updated)
			throws IllegalArgumentException, InterruptedException {
		assertTrue("Client original started in active",
				controller.exists(activePath + original.getClientID()));
		assertEquals("Client to update has same ID as original", original.getClientID(),
				updated.getClientID());

		@SuppressWarnings("unchecked")
		Response<Boolean> response = (Response<Boolean>) TestUtil.run(Command.CLIENT_CONFIG_UPDATE,
				updated, sender, controller);
	...
	private void deleteClient(ClientID id) throws IllegalArgumentException, InterruptedException {
		...
		@SuppressWarnings("unchecked")
		Response<Boolean> response = (Response<Boolean>) TestUtil.run(Command.CLIENT_CONFIG_DELETE,
