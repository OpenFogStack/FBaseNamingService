...
public class NodeTest {
	...
	private void createNode(NodeConfig c) throws IllegalArgumentException, InterruptedException {
		...
		assertFalse("Node not in tombstoned at start",
				controller.exists(tombstonedPath + c.getNodeID()));

		@SuppressWarnings("unchecked")
		Response<Boolean> response = (Response<Boolean>) TestUtil.run(Command.NODE_CONFIG_CREATE, c,
				c.getID(), controller);
	...
	private void readNode(NodeID id, NodeConfig expected)
			throws IllegalArgumentException, InterruptedException {
		assertTrue("Node started in active", controller.exists(activePath + id));

		@SuppressWarnings("unchecked")
		Response<String> response =
				(Response<String>) TestUtil.run(Command.NODE_CONFIG_READ, id, id, controller);
		...
		assertEquals("Reads correct algorithm", expected.getEncryptionAlgorithm(),
				r.getEncryptionAlgorithm());
		assertEquals("Reads correct machines", expected.getMachines(), r.getMachines());
		assertEquals("Reads correct publisher port", expected.getPublisherPort(),
				r.getPublisherPort());
	...
	private void updateNode(NodeConfig original, NodeConfig updated)
			throws IllegalArgumentException, InterruptedException {
		assertTrue("Node original started in active",
				controller.exists(activePath + original.getNodeID()));
		assertEquals("Node to update has same ID as original", original.getNodeID(),
				updated.getNodeID());

		@SuppressWarnings("unchecked")
		Response<Boolean> response = (Response<Boolean>) TestUtil.run(Command.NODE_CONFIG_UPDATE,
				updated, original.getID(), controller);
	...
	private void deleteNode(NodeID id) throws IllegalArgumentException, InterruptedException {
		...
		Response<Boolean> response =
				(Response<Boolean>) TestUtil.run(Command.NODE_CONFIG_DELETE, id, id, controller);
	...
	private void updateNodeDifferentSender(NodeConfig original, NodeConfig updated)
			throws IllegalArgumentException, InterruptedException {
		assertTrue("Node original started in active",
				controller.exists(activePath + original.getNodeID()));
		assertEquals("Node to update has same ID as original", original.getNodeID(),
		...
		@SuppressWarnings("unchecked")
		Response<Boolean> response = (Response<Boolean>) TestUtil.run(Command.NODE_CONFIG_UPDATE,
				updated, new NodeID("wrong_id"), controller);

		assertFalse("Proper failure response", response.getValue());
		assertEquals("Proper response code", ResponseCode.ERROR_ILLEGAL_COMMAND,
				response.getResponseCode());
	}

	private void deleteNodeDifferentSender(NodeID id)
			throws IllegalArgumentException, InterruptedException {
		assertTrue("Node started in active", controller.exists(activePath + id));

		@SuppressWarnings("unchecked")
		Response<Boolean> response = (Response<Boolean>) TestUtil.run(Command.NODE_CONFIG_DELETE,
				id, new NodeID("wrong_id"), controller);
	...
	private NodeConfig makeUpdatedNode(NodeConfig n) {
		...
		return new NodeConfig(n.getID(), key2, alg2, machines2, pPort2, mPort2, rPort2, loc2,
				desc2);
	}
}
