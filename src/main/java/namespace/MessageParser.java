...
public class MessageParser {
	...
	private static Response<?> nodeUpdate(IControllable controller, String content,
			NodeID senderID) {
		NodeConfig node = JSONable.fromJSON(content, NodeConfig.class);
	...
	private static Response<?> nodeDelete(IControllable controller, String content,
			NodeID senderID) {
		NodeID nodeID = JSONable.fromJSON(content, NodeID.class);
	...
	private static Response<?> keygroupAddReplicaNode(IControllable controller, String content,
			NodeID senderID) {
		...
		String keygroupJSON =
				Keygroup.getInstance().getKeygroupInfoAuthorized(controller, keygroupID).getValue();
		KeygroupConfig keygroup = JSONable.fromJSON(keygroupJSON, KeygroupConfig.class);

		if (keygroup.containsReplicaNode(senderID)) {
			return Keygroup.getInstance().addReplicaNodeToKeygroup(controller, replicaNode,
	...
	private static Response<?> keygroupAddTriggerNode(IControllable controller, String content,
			NodeID senderID) {
		// Get KeygroupID and node from JSON via wrapper
		ConfigToKeygroupWrapper wrapper = JSONable.fromJSON(content, ConfigToKeygroupWrapper.class);
		KeygroupID keygroupID = wrapper.getKeygroupID();
		...
		if (keygroup.containsReplicaNode(senderID)) {
			return Keygroup.getInstance().addTriggerNodeToKeygroup(controller, triggerNode,
					keygroup.getKeygroupID());
		} else {
			return new Response<Boolean>(false, ResponseCode.ERROR_ILLEGAL_COMMAND);
		}
	...
	private static Response<?> keygroupRead(IControllable controller, String content,
			NodeID senderID) {
		KeygroupConfig keygroup = JSONable.fromJSON(content, KeygroupConfig.class);
		if (keygroup.containsReplicaNode(senderID) || keygroup.containsTriggerNode(senderID)) {
			return Keygroup.getInstance().getKeygroupInfoAuthorized(controller,
					keygroup.getKeygroupID());
		} else {
			return Keygroup.getInstance().getKeygroupInfoUnauthorized(controller,
					keygroup.getKeygroupID());
		}
	}

	private static Response<?> keygroupUpdateCrypto(IControllable controller, String content,
			NodeID senderID) {
		// Get KeygroupID and node from JSON via wrapper
		...
		if (keygroup.containsReplicaNode(senderID)) {
			return Keygroup.getInstance().updateKeygroupCrypto(controller, keygroupID,
					encryptionSecret, encryptionAlgorithm);
		} else {
			return new Response<Boolean>(false, ResponseCode.ERROR_ILLEGAL_COMMAND);
		}
	}

	private static Response<?> keygroupDelete(IControllable controller, String content,
			NodeID senderID) {
