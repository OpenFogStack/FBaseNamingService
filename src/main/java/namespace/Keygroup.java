...
/**
 * The Keygroup class performs all operations on the Keygroups section of the system. This is
 * primarily designed to keep track of information about how nodes are logically structured
 * together in the system to form a fog network.
 * 
...
public class Keygroup extends SystemEntity {
	 ...
	 * @return Response object with Boolean containing the success or failure of operation
	 */
	Response<Boolean> addReplicaNodeToKeygroup(IControllable controller, ReplicaNodeConfig rnode,
			KeygroupID keygroupID) {
		return addNodeToKeygroup(controller, rnode, keygroupID,
				(keygroupConfig, node) -> keygroupConfig.addReplicaNode((ReplicaNodeConfig) node));
	}
	 ...
	 * @return Response object with Boolean containing the success or failure of operation
	 */
	Response<Boolean> addTriggerNodeToKeygroup(IControllable controller, TriggerNodeConfig tNode,
			KeygroupID keygroupID) {
		return addNodeToKeygroup(controller, tNode, keygroupID,
				(keygroupConfig, node) -> keygroupConfig.addTriggerNode((TriggerNodeConfig) node));
	}
	 ...
	 * @return Response object with Boolean containing the success or failure of operation
	 */
	private Response<Boolean> addNodeToKeygroup(IControllable controller, KeygroupMember node,
			KeygroupID keygroupID, BiConsumer<KeygroupConfig, KeygroupMember> addToList) {
		try {
	 ...
	 * @return Response object with Boolean containing the success or failure of operation
	 */
	Response<Boolean> removeNodeFromKeygroup(IControllable controller, NodeID nodeID,
			KeygroupID keygroupID) {
		try {
			if (isActive(controller, keygroupID.toString())) {
	...
	/**
	 * Responds with all information about the Keygroup. This includes the encryption key and
	 * algorithm, so can only be called by nodes in the Keygroup.
	 * 
	 * @param controller Controller for interfacing with base distributed system
	...
	/**
	 * Responds with all information about the Keygroup. This does not includes the encryption
	 * key and algorithm, so can be used by any node in the system.
	 * 
	 * @param controller Controller for interfacing with base distributed system
	 * @param keygroupID The ID to the Keygroup to get information from
	 * @return Response object with String containing the Keygroup information
	...
	/**
	 * Updates the encryption key and algorithm to be used for communication within the
	 * Keygroup.
	 * 
	 * @param controller Controller for interfacing with base distributed system
	 ...
	 * @param encryptionAlgorithm Encryption algorithm (symmetric) used for communication
	 *            within the Keygroup
	 * @return Response object with Boolean containing the success or failure of operation
	 */
	Response<Boolean> updateKeygroupCrypto(IControllable controller, KeygroupID keygroupID,
			String encryptionSecret, EncryptionAlgorithm encryptionAlgorithm) {
		try {
			if (isActive(controller, keygroupID.toString())) {
	 ...
	 */
	private Response<Boolean> removeNodeFromActiveKeygroup(IControllable controller, NodeID nodeID,
			KeygroupID keygroupID) throws KeeperException, InterruptedException {
		String data = controller.readNode(activePath(keygroupID.toString()));
	 ...
	 * @throws InterruptedException
	 */
	private Response<Boolean> removeNodeFromTombstonedKeygroup(IControllable controller,
			NodeID nodeID, KeygroupID keygroupID) throws KeeperException, InterruptedException {
		...
		if (keygroup.containsReplicaNode(nodeID)) {
			...
			if (keygroup.getReplicaNodes().size() == 1) {
				// If no remaining trigger nodes, destroy group, otherwise, send error until
				// trigger node list is empty
	 ...
	 * @throws InterruptedException
	 */
	private Response<Boolean> destroyKeygroup(IControllable controller, KeygroupID keygroupID)
